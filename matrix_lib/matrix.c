...
MATRIX_EXCEPTION malloc_matrix(Matrix *matrix, size_t rows, size_t cols)
{
    ...
    if (rows * cols * sizeof(double) > sizeof(size_t)){
        return EXC_OVERFLOW;
    }
...
MATRIX_EXCEPTION summarize(Matrix *resl_mtx, const Matrix *mtx1, const Matrix *mtx2)
{
    ...
    for(size_t i = 0; i < mtx1->cols * mtx1->rows; i += cols)
    {
        // i += cols equals to row += 1
        for(size_t j = i; j < cols + i; j++)
        {
...
void add(const Matrix *basic, const Matrix *adding)
{
    ...
    for(size_t i = 0; i < basic->cols * basic->rows; i += cols)
    {
        // i += cols equals to row += 1
        for(size_t j = i; j < cols + i; j++)
        {
...
MATRIX_EXCEPTION subtract(Matrix *resl_mtx, const Matrix *mtx, const Matrix *sub_mtx)
{
    ...
    for(size_t i = 0; i < mtx->cols * mtx->rows; i += cols)
    {
        // i += cols equals to row += 1
        for(size_t j = i; j < cols + i; j++)
        {
...
void multiply_by_number(Matrix *matrix, double number)
{
    ...
    for(size_t i = 0; i < matrix->cols * matrix->rows; i += cols)
    {
        // i += cols equals to row += 1
        for(size_t j = i; j < cols + i; j++)
        {
...
// mtx must be initialized
double matrix_sum_diagonal(Matrix *mtx)
{
    ...
    double result = 0.0;
    for(size_t idx = 0; idx < mtx->cols * mtx->rows; idx += mtx->cols + 1)
    {
        result += data[idx];
    }
    return result;
